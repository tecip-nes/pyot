...
/*----------------------------------------------------------------------------*/
/**
 * Imports a module of the given name from image passed as parameter.
 * A code obj is created for the code image.
 * A module obj is created for the code obj.
 *
 * @param   pstr String obj containing name of code obj to load.
 * @param   pmod Return by reference; ptr to imported module
 * @param   memspace  Memory space in which th image containing the module 
 *          is located
 * @param   imgaddr   Address of the image containing the module
 * @return  Return status
 */
PmReturn_t mod_import_from_img(pPmObj_t pstr, pPmObj_t *pmod, 
        PmMemSpace_t memspace, uint8_t const *imgaddr);
